...
public readonly record struct RamsesNote(RamsesNoteType Type, int Pos)
{
	...
	public static RamsesNote? TryFrom(JsonNote note)
	{
		if (Enum.IsDefined(note.Direction)
		    ...
		    && note.Y is >= 0 and <= 2)
		{
			return new RamsesNote((RamsesNoteType)((note.Type == NoteColor.Red ? 0 : 9) + note.Direction),
				PosFromLoc(note.X, note.Y));
		}
	...
	{
		if (x is < 0 or > 3)
			throw new ArgumentOutOfRangeException(nameof(x), "Position out of bounds");
		if (y is < 0 or > 2)
			throw new ArgumentOutOfRangeException(nameof(y), "Position out of bounds");
		return x + y * 4;
	}
